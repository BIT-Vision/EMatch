...
def get_args_parser():
    ...
    parser.add_argument('--configs_dataset', default='./datasets/configs_dataset/ematch/disparity/mvsec_train_split1.yaml', type=str)
    ...
    # training
    parser.add_argument('--num_steps', default=150000, type=int)
...
def main(args):
    ...
    while total_steps < args.num_steps:
        ...
        for i, sample in enumerate(train_loader):
            ...
            # print(disparity_gt[0])
            # print(disparity_gt[0].shape)
            # print(torch.max(disparity_gt[0][disparity_gt[0]!=float('inf')]))
            # from utils.disparity_vis import tensor_to_disparity_magma_image # , _make_disparity_image, _save_disparity_image
            # disparity_pred = tensor_to_disparity_magma_image(disparity_gt[0].cpu().detach(), vmax=100)
            # disparity_pred.save('./test.png')
            # # _save_disparity_image('./test.png', _make_disparity_image(disparity_gt[0].cpu().detach().numpy(), 256, 256))
            ...
            logger.add_disparity_summary(disparity_preds[-1][0], 'train/disparity_predicted', vmax=37)
            logger.add_disparity_summary(disparity_gt[0], 'train/disparity_gt', vmax=37)
