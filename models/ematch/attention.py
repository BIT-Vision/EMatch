...
class RefineBlock(nn.Module):
    def __init__(self, flow_channels, context_channels, emb_dim):
        super(RefineBlock, self).__init__()
        self.proj_flow = nn.Conv2d(flow_channels, emb_dim, kernel_size=1, stride=1, padding=0)
        self.proj_context = nn.Conv2d(context_channels, emb_dim, kernel_size=1, stride=1, padding=0)

        self.q_proj = nn.Linear(emb_dim, emb_dim)
        self.k_proj = nn.Linear(emb_dim, emb_dim)
        self.v_proj = nn.Linear(emb_dim, flow_channels)

    def forward(self, flow, context):
        # q
        q = self.proj_context(context) # [B, emb_dim, H, W]
        b, c, h, w = q.size()
        query = self.q_proj(q.view(b, c, h * w).permute(0, 2, 1)) # [B, H*W, emb_dim]

        # k
        kv = self.proj_flow(flow) # [B, emb_dim, H, W]
        b, c, h, w = kv.size()
        kv = kv.view(b, c, h * w).permute(0, 2, 1)
        key = self.k_proj(kv) # [B, H*W, emb_dim]
