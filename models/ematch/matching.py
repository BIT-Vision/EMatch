...
def global_correlation_softmax(feature0, feature1, pred_bidir_flow=False):
    ...
    if matching == 'soft':
        ...
        prob = (correlation == correlation.max(dim=1, keepdim=True)[0]).to(dtype=torch.float32) # [B, H*W, H*W]
...
def local_correlation_softmax_stereo(feature0, feature1, local_radius,
    ...
    # exclude coords that are out of image space
    valid_x = (sample_coords[:, :, :, 0] >= 0) & (sample_coords[:, :, :, 0] < w)  # [B, H*W, (2R+1)]
    valid_y = (sample_coords[:, :, :, 1] >= 0) & (sample_coords[:, :, :, 1] < h)  # [B, H*W, (2R+1)]

    valid = valid_x & valid_y  # [B, H*W, (2R+1)], used to mask out invalid values when softmax
    ...
    correspondence = torch.matmul(prob.unsqueeze(-2), sample_coords_softmax).squeeze(-2).view(b, h, w, 2).permute(0, 3, 1, 2).contiguous()  # [B, 2, H, W]
