...
def save_cfgs(cfgs, filename='cfgs.json'):
    check_path(cfgs.checkpoint_dir)
    save_path = os.path.join(cfgs.checkpoint_dir, filename)

    # Save all training args when resuming training
    with open(save_path, 'a') as f:
        json.dump(cfgs, f, indent=4, sort_keys=False)
        f.write('\n\n')
