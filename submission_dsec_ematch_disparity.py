...
def get_args_parser():
    ...
    parser.add_argument('--configs_dataset', default='./datasets/configs_dataset/ematch/disparity/dsec_test.yaml', type=str)
...
def main(cfgs):
    ...
    for i, name in enumerate(names):
        ...
        for i in tqdm(range(len(test_dataset))):
            ...
            if cfgs.visual:
                disparity_pred = tensor_to_disparity_magma_image(disparity_preds.cpu().detach(), vmax=100)
                disparity_pred.save(os.path.join(visual_path, str(file_index).zfill(6) + '.png'))

            # Save
            disparity = results_dict['disparity_preds'][-1][0].cpu().detach().numpy()
            disparity = (disparity * 256).astype(np.uint16)
            cv2.imwrite(os.path.join(submission_path, str(file_index).zfill(6) + '.png'), disparity)
